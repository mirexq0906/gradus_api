...
import { fetchCall } from "../../../API/FeedbackService";
 
const CallAdmin = () => {
  const [call, setCall] = useState([]);

  async function FetchAllCall() {
  ...
  async function deleteBlog(blogId) {
    await axios.delete(process.env.REACT_APP_SERVER + 'call_clients/' + blogId);
              ...
              <tr key={item.id}>
                ...
                <td className="table-admin__body-buttons">
                  <div className="table-admin__row-buttons">
                    <button onClick={() => deleteBlog(item.id)} className="btn table-admin__delete">
                      <svg
